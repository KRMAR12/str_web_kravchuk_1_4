...
link_for_scarpping = "https://en.wikipedia.org/wiki/Legality_of_cryptocurrency_by_country_or_territory"

# 1. Disguise the script as a normal Google Chrome browser
headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'
}

# Fetch the webpage using the disguise
response = requests.get(link_for_scarpping, headers=headers)

# 2. Extract ALL tables from the page
all_tables = pd.read_html(response.text)

# 3. Filter out the junk (sidebars, info-boxes) and keep ONLY tables with a 'Legality' column
crypto_tables = [df for df in all_tables if 'Legality' in df.columns]

# 4. Combine all the regional tables into one master DataFrame
status_by_country = pd.concat(crypto_tables, axis=0, ignore_index=True)
